...
```
## link linc with bike data
```{r}
boxp <- bikeshare_start %>% rename(point_start = geometry) %>% mutate(lat_linc = point_start) %>% mutate(long_linc = lat_linc)

boxp[,'lat_linc'] = linc_lat
boxp[,'long_linc'] = linc_long

boxp1 <- bikeshare %>% select(member_casual, start_lng, start_lat)

boxp2 <- full_join(boxp, boxp1, by = 'member_casual')
```
...
## distance from linc
```{r}
linc_dist <- bikeshare_start %>% mutate(distance = haversine(geometry, linc_latlong, round = 4))
```
