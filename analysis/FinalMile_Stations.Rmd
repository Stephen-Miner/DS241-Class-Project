...
metro_stations = read_csv(here("data_raw", "dc_metro_stations.csv")) %>%
  clean_names()
metro_stations_sf = geojson_sf(here("data_raw", "dc_metro_stations.geojson")) %>%
...
As someone who doesn't know the DC area very well, I don't have that many observations here - I think the spatial viz down below where it shows these counts on the map is more useful.
...
This viz shows the same information as the one above titled "Most Popular DC Metro Stations for 'Final Mile' Bikeshare Rides", but on a map instead of a simple bar chart.

```{r}
tmap_mode("view")
tm_basemap(c(StreetMap = "OpenStreetMap")) + tm_shape(final_mile1_geo) +
  tm_dots(size = "count", alpha = .25)
```

It makes sense that the metro stations around the more populated working area of DC have more bikeshare rides near them. Interesting to note, though, that each individual metro station still has more close bikeshare rides despite the fact that there are more of them in a smaller area than the ones towards the outskirts.
...
metro_stations_separate = metro_stations_sf %>% mutate(
...
```{r}
tm_shape(final_mile2) + tm_lines(lwd="count", scale = 5) +
  tm_layout(title = "Metro Stations and Bikeshare Stations Within 500 ft of Each Other")
```

I think the main difference between what we can see with this map versus the other one is not only where the nearby bikeshare stations are, but also if there are multiple (e.g., Dupont Circle, Foggy Bottom). Columbia Heights is actually a really interesting example of this, where there are two nearby bikeshare stations but one gets used a good deal more than the other.

## Output data

Write some of our dataframes to csv for potential future use by others. 
```{r}
final_mile1 %>% rename("close_metro_station" = "close_station",
                                     "dist_metro_to_bikeshare" = "distance",
                                     "bike_ride_dist" = "start_to_end") %>%
  write_csv(file = here("data", "final_mile_rides.csv"), append = FALSE)
final_mile1_grouped %>% 
  rename("close_metro_station" = "close_station",
         "num_rides" = "count") %>% 
  write_csv(file = here("data", "final_mile_metro.csv"), append = FALSE)
final_mile1_geo %>% rename("close_metro_station" = "close_station",
                                                  "num_rides" = "count") %>%
  st_write(here("data", "final_mile_metro.geojson"), append = FALSE)
final_mile2 %>% select(-mlat, -mlng, -blat, -blng) %>% 
  rename("close_metro_station" = "close_station",
         "num_rides" = "count",
         "metro_to_bikeshare" = "geometry") %>%
  st_write(here("data", "close_metro_bikeshare_stations.geojson"), append = FALSE)
```
